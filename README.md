...
This supports DUB's default `source/` and `src/` layouts, including projects
like `libasync` that define a static library at the repository root and
executables under nested example or test packages.
...
`rules_d` DUB repository, for example `@dub//memutils`.
